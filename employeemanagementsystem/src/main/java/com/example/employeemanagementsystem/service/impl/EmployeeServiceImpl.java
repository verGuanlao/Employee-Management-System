...
public class EmployeeServiceImpl implements EmployeeService {
    ...
    public EmployeeStatsResponse getEmployeesWithStats(Long deptId, Integer minAge, Integer maxAge, Pageable pageable) {
            ...
            if (deptId != null && minAge != null && maxAge != null) {
                page = employeeRepository.findByDepartmentIdAndBirthDateBetween(deptId, minBirthDate, maxBirthDate, pageable);
                ...
                page = employeeRepository.findByDepartmentId(deptId, pageable);
                ...
                page = employeeRepository.findByBirthDateBetween(maxBirthDate, maxBirthDate, pageable);
                avgSalary = employeeRepository.calculateAverageSalaryByAgeRange(maxBirthDate, maxBirthDate);
                avgAge = employeeRepository.calculateAverageAgeByRange(maxBirthDate, maxBirthDate);
            } else if (maxAge != null) {
                page = employeeRepository.findByBirthDateBetween(minBirthDate, minBirthDate, pageable);
                avgSalary = employeeRepository.calculateAverageSalaryByAgeRange(minBirthDate, minBirthDate);
                avgAge = employeeRepository.calculateAverageAgeByRange(minBirthDate, minBirthDate);
            } else {
